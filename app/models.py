...
class FaceEmbedding(db.Model):
    ...
    def set_face_box(self, box):
        ...
        if isinstance(box, (list, tuple)):
            box = [int(x) if hasattr(x, 'item') else x for x in box]
...
class TestImage(db.Model):
    """Test images with ground-truth annotations for detector evaluation"""
    ...
    # Relationships
    annotations = db.relationship('GroundTruthAnnotation', backref='image', lazy='dynamic', cascade='all, delete-orphan')
    
    def __repr__(self):
        return f'<TestImage {self.filename}>'


class GroundTruthAnnotation(db.Model):
    """Ground-truth face bounding boxes for test images"""
    __tablename__ = 'ground_truth_annotations'
    
    id = db.Column(db.Integer, primary_key=True)
    test_image_id = db.Column(db.Integer, db.ForeignKey('test_images.id'), nullable=False)
    x = db.Column(db.Integer, nullable=False)  # Top-left x coordinate
    y = db.Column(db.Integer, nullable=False)  # Top-left y coordinate
    width = db.Column(db.Integer, nullable=False)
    height = db.Column(db.Integer, nullable=False)
    notes = db.Column(db.String(256))  # Optional: person name, face attributes, etc.
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    ...
    def __repr__(self):
        return f'<GroundTruth Image {self.test_image_id} [{self.x},{self.y},{self.width},{self.height}]>'


class DetectorEvaluation(db.Model):
    """Stores evaluation results for face detectors"""
    __tablename__ = 'detector_evaluations'
    
    id = db.Column(db.Integer, primary_key=True)
    detector_name = db.Column(db.String(32), nullable=False)  # 'mtcnn', 'haar', 'dnn', 'retinaface'
    
    # Dataset information
    dataset_name = db.Column(db.String(128))
    num_images = db.Column(db.Integer, nullable=False)
    num_ground_truth_faces = db.Column(db.Integer, nullable=False)
    num_detected_faces = db.Column(db.Integer, nullable=False)
    iou_threshold = db.Column(db.Float, default=0.5)
    confidence_threshold = db.Column(db.Float, default=0.7)
    
    # Detection metrics
    true_positives = db.Column(db.Integer, nullable=False)
    false_positives = db.Column(db.Integer, nullable=False)
    false_negatives = db.Column(db.Integer, nullable=False)
    ...
    # Performance metrics
    avg_processing_time_ms = db.Column(db.Float)  # Average time per image
    total_processing_time_ms = db.Column(db.Float)  # Total time for all images
    ...
    def __repr__(self):
        return f'<DetectorEvaluation {self.detector_name} F1={self.f1_score:.2f}>'


class EvaluationRun(db.Model):
    """Tracks complete evaluation runs across all detectors"""
    ...
    num_ground_truth_faces = db.Column(db.Integer)
    iou_threshold = db.Column(db.Float, default=0.5)
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)
    ...
    def __repr__(self):
        return f'<EvaluationRun {self.id} {self.timestamp}>'
